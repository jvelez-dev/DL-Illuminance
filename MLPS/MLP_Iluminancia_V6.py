# -*- coding: utf-8 -*-
"""
Created on Tue Mar  8 19:00:04 2022

Entrenar modelo 73 sin EH ni EV y 2 focos
...
EPOCHS = 5000
PACIENCIA = 80
CARPETA = 'C:/Users/JVelez/TF_MLP/modelos/Version6/'
IV_ENTRENAMIENTO=''
IV_VALIDACION=''
IV_EVALUACION=''
IV_PREDICCION=''
...
def cargar_conjunto(conjunto):
    ...
    df.columns=['FocoX','FocoY','FocoZ','FocoI','Foco2X','Foco2Y','Foco2Z','Foco2I','PuntoX','PuntoY','PuntoZ','Distancia','EH','EV','PuntoI']
...
def build_model(N,LR):
    ...
    model.add(layers.Dense(units=N,activation='relu',input_dim=12))
...
def generacion_cubo_predicciones(dataset,nombre_archivo,nuevo_modelo):  
    ...
    df.columns=['FocoX','FocoY','FocoZ','FocoI','Foco2X','Foco2Y','Foco2Z','Foco2I','PuntoX','PuntoY','PuntoZ','Distancia','PuntoI']
    ...
    np.savetxt("C:/Users/JVelez/TF_MLP/modelos/Version6/cubo_predicciones_{}.csv".format(nombre_archivo),df_result,delimiter=';',fmt='%f')
...
def cargar_modelo(model_name):
    nuevo_modelo = keras.models.load_model('C:/Users/JVelez/TF_MLP/modelos/Version6/{}/{}.h5'.format(model_name,model_name)) 
    return nuevo_modelo   
...
def construir_entrenar_validar():    
   X,Y=cargar_conjunto("dataset_VL_11_2Focos")
   global MIN 
   MIN = np.min(Y)
   global MAX
   MAX = np.max(Y)
...
def registrar_reporte(model_id,history,eval_score,predict_score,tiempo):
    ...
    with open('./modelos/Version6/reporte.csv', 'a') as f_object:
...
def generar_modelo(i):
    ...
    error_rmse,error_mae=probar_predicciones(modelo, 'dataset_432_VL_2Focos_PRED')
...
# generar_modelo(74)
#modelo = cargar_modelo("modelo_74")
# generar_errores_prediccion_escala_real(modelo,"dataset_432_VL_PRED_EHEV")
# generar_errores_escala_real(modelo)
#generacion_cubo_predicciones("dataset_432_VL_2Focos_PRED", "modelo_74", modelo)
