...
EPOCHS = 5000
PACIENCIA = 80
CARPETA = 'C:/Users/JVelez/TF_MLP/modelos/Version5_1/'
IV_ENTRENAMIENTO=''
IV_VALIDACION=''
IV_EVALUACION=''
IV_PREDICCION=''
...
def build_model(N,LR):
    ...
    model.add(layers.Dense(units=N,activation='relu',input_dim=14))
...
def generacion_cubo_predicciones(dataset,nombre_archivo,nuevo_modelo):  
    ...
    np.savetxt("C:/Users/JVelez/TF_MLP/modelos/Version5_1/cubo_predicciones_{}.csv".format(nombre_archivo),df_result,delimiter=';',fmt='%f')
...
def cargar_modelo(model_name):
    nuevo_modelo = keras.models.load_model('C:/Users/JVelez/TF_MLP/modelos/Version5_1/{}/{}.h5'.format(model_name,model_name)) 
    return nuevo_modelo   
...
def registrar_reporte(model_id,history,eval_score,predict_score,tiempo):
    ...
    with open('./modelos/Version5_1/reporte.csv', 'a') as f_object:
...
def generar_modelo(i):
    ...
    error_rmse,error_mae=probar_predicciones(modelo, 'dataset_VL_11_2Focos')
...
def generar_RMSPE():
    modelo = cargar_modelo('modelo_80')
    X,Y=cargar_conjunto("dataset_VL_11_2Focos")
    scaler_x = MinMaxScaler()
    scaler_y = MinMaxScaler()
    x_scale = scaler_x.fit_transform(X)
