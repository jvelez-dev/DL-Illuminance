...
Entrenar modelo 72 sin EH ni EV
...
def cargar_conjunto(conjunto):
    ...
    df.columns=['FocoX','FocoY','FocoZ','FocoI','PuntoX','PuntoY','PuntoZ','Distancia','PuntoI']
...
def build_model(N,LR):
    ...
    model.add(layers.Dense(units=N,activation='relu',input_dim=8))
...
def generacion_cubo_predicciones(dataset,nombre_archivo,nuevo_modelo):  
    ...
    np.savetxt("C:/Users/JVelez/TF_MLP/modelos/Version5/cubo_predicciones_{}.csv".format(nombre_archivo),df_result,delimiter=';',fmt='%f')
...
def construir_entrenar_validar():    
   X,Y=cargar_conjunto("dataset_VL_11_EHEV")
   global MIN 
   MIN = np.min(Y)
   global MAX
   MAX = np.max(Y)
...
def generar_modelo(i):
    ...
    error_rmse,error_mae=probar_predicciones(modelo, 'dataset_432_VL_PRED_EHEV')
...
def generar_conjuntos_sin_EHEV():
    df = pd.read_csv (r'.\datasets\dataset_VL_11.csv', sep=';', header=None)
    df.columns=['FocoX','FocoY','FocoZ','FocoI','PuntoX','PuntoY','PuntoZ','Distancia','EH','EV','PuntoI']
    df = df.drop(columns=['EH','EV'])
    df.to_csv("dataset_VL_11_EHEV.csv", header=None, index=False, sep=";")
    
    df = pd.read_csv (r'.\datasets\dataset_432_VL_PRED.csv', sep=';', header=None)
    df.columns=['FocoX','FocoY','FocoZ','FocoI','PuntoX','PuntoY','PuntoZ','Distancia','EH','EV','PuntoI']
    df = df.drop(columns=['EH','EV'])
    df.to_csv("dataset_432_VL_PRED_EHEV.csv", header=None, index=False, sep=";")
...
generacion_cubo_predicciones("dataset_432_VL_PRED_EHEV", "modelo_73", modelo)
